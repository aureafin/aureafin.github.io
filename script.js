...
(() => {
  ...
  function startTask(index) {
    ...
    if (taskSocialBarMoment(index) === 'start') openAdWall(launch);
    else launch();
  }
  ...
  function completeTask(index) {
    ...
    if (taskSocialBarMoment(index) === 'end') openAdWall(showReward);
    else showReward();
    return true;
  }
  ...
  function startSuper() {
    ...
    if (superSocialBarMoment() === 'start') openAdWall(launch);
    else launch();
  }
  ...
  function completeSuper() {
    ...
    if (showAtEnd) openAdWall(showReward);
    else showReward();
    return true;
  }
  ...
  /* ------------------------------------------------------
     ...
     - Social Bar: dispara junto com o ecrã de publicidade em tela
       cheia (#adWall), dividido de forma proporcional entre o início
       e o fim das tarefas (metade/metade) e da Super Tarefa.
     - Ecrã de tela cheia (#adWall): mostra o anúncio 300x250 e só
       deixa sair depois de CONFIG.AD_WALL_SECONDS segundos.
     - Native Banner: dentro de #adSlotTask (modal pós-tarefa).
     - Banner 320x50: fixo em #adSlotBannerTop.
     - Popunder: dispara sozinho ao clique, carregado uma vez no HTML.
     ------------------------------------------------------ */
  ...
  // Divide as 10 tarefas ao meio: índices pares mostram o ecrã de
  // publicidade ao INÍCIO, índices ímpares mostram-no ao FIM. 5 de
  // cada, proporcional.
  ...
  // Ecrã de publicidade em tela cheia (Social Bar + Banner 300x250).
  // onDone só é chamado depois de o utilizador esperar o tempo
  // definido e clicar em "Sair".
  let adWallTimerId = null;

  function openAdWall(onDone) {
    const wall = $('#adWall');
    const btn = $('#adWallExitBtn');
    const label = $('#adWallExitLabel');
    if (!wall || !btn || !label) { if (onDone) onDone(); return; }

    triggerSocialBar();

    let remaining = CONFIG.AD_WALL_SECONDS;
    btn.disabled = true;
    btn.classList.remove('is-ready');
    label.textContent = `Aguarde ${remaining}s`;

    if (adWallTimerId) clearInterval(adWallTimerId);
    adWallTimerId = setInterval(() => {
      remaining -= 1;
      if (remaining > 0) {
        label.textContent = `Aguarde ${remaining}s`;
      } else {
        clearInterval(adWallTimerId);
        adWallTimerId = null;
        btn.disabled = false;
    ...
    const finish = () => {
      ...
      if (adWallTimerId) { clearInterval(adWallTimerId); adWallTimerId = null; }
      wall.classList.remove('show');
      setTimeout(() => { wall.hidden = true; }, 220);
